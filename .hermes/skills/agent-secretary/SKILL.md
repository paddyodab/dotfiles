...
metadata:
  hermes:
    tags: [agent, secretary, clerical, commits, prs]
---
...
- For `COMMIT` tasks, load the `provable-commits` skill via `skill_view(name="provable-commits")`.
- For `CR` tasks, load the `shortcut-cr` skill via `skill_view(name="shortcut-cr")`.
