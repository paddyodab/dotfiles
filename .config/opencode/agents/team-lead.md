...
prompt: "You have a blocking message in your inbox from team-lead.
...
Spawn reviewer (`subagent_type: reviewer`), then after Task returns:
```bash
...
Spawn coder (`subagent_type: coder`), read reply, validate `CHANGES_MADE`, `TESTS`, `DEVIATIONS`, `VALIDATION`, `COMMIT_SHA` present.
...
Delegate to Secretary (`subagent_type: secretary`):
