...
cmd_install() {
    ...
    echo ""
    echo "  OpenCode:"
    echo "    Agents: ~/.config/opencode/agents/ (9 agents with model tiering)"
    echo "            team-lead (primary), planner, coder, reviewer, secretary (hidden),"
    echo "            puddleglum, doc-agent, investigator, issue-worker (hidden)"
    echo "    Skills: agent-message-bus, log-decision, model-advisor"
